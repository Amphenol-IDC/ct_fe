...
// support printf
#ifdef __GNUC__
  /* With GCC/RAISONANCE, small printf (option LD Linker->Libraries->Small printf) calls __io_putchar() */
...
void print_reset_reason(void)
{
    ...
    printf("\r\n===== RESET REASON: 0x%08lX =====\r\n", rsr);

    if (rsr & RCC_RSR_WWDGRSTF) printf("Window Watchdog Reset\r\n");
    if (rsr & RCC_RSR_IWDGRSTF) printf("Independent Watchdog Reset\r\n");
    if (rsr & RCC_RSR_SFTRSTF)  printf("Software Reset\r\n");
    if (rsr & RCC_RSR_BORRSTF)  printf("Power/Brown-Out Reset\r\n");
    if (rsr & RCC_RSR_PINRSTF)  printf("Pin Reset (NRST)\r\n");
    if (rsr & RCC_RSR_LPWRRSTF) printf("Low-Power Reset\r\n");
...
int main(void)
{
  ...
  /* Initialize SPI4 slave register map */
  SpiRegs_Init();

  /* Initialize 32 channel P/N GPIO pairs (64 GPIOs) */
// I will do the init in ioc file later on  GpioChannels_Init();
...
static void MX_IWDG_Init(void)
{
  ...
  return; // disable watchdog for testing...............TBD later
...
void StartWDTask(ULONG argument)
{
    uint8_t led_state = 0; // 0=green, 1=yellow, 2=red

    /* Logging snapshots: this is the ONLY thread that calls printf at run time,
     * so printf's large/blocking stack usage never lands in the 1 ms RT thread. */
    ...
    for (;;)
    {
        /* --- Drain deferred SPI log events (safe: 500 ms, big-enough stack) --- */
        ...
        // Turn on the current LED based on state
        switch(led_state)
        {
            case 0:
